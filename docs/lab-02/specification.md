...
- **FR-04 (Ticket Number Generation):** Upon valid submission, the backend MUST generate a unique, non-sequential official Ticket Number in format `TKT-YYYY-XXXXXX` (e.g. `TKT-2026-000101`).
...
- **FR-10 (Sorting & Pagination):** The ticket list MUST support sorting by creation date and ticket number, and pagination with page numbers and item count display.
...
- **BR-01 (Ticket Number Format):** Official Ticket Numbers are generated exclusively by the backend upon successful creation and MUST follow the format `TKT-YYYY-XXXXXX` (where `YYYY` is the current year and `XXXXXX` is a zero-padded sequential sequence number).
...
- **BR-05 (Ownership Isolation):** Requesters can ONLY view, search, open, upload attachments to, or soft-remove attachments from tickets they own (`requesterId === selectedRequester.id`). Direct requests to access or modify another requester's ticket or attachment MUST be rejected with HTTP 404 Not Found or 403 Forbidden.
...
- **BR-09 (Soft Removal Rules):** Attachment removal MUST be implemented as soft removal (`isRemoved = true`, `removedAt = timestamp`). Soft-removed attachments MUST remain visible as metadata in the attachment list marked as "Removed", but file downloading or previewing MUST be permanently blocked.
...
- **BR-12 (Query Defaults):** The default ticket list query MUST return `page=1`, `limit=10`, sorted by `createdAt` descending.
- **BR-13 (Inactive User Exclusion):** If a user ID corresponds to an inactive requester (`isActive = false`), ticket creation or ticket list fetching for that ID MUST fail with an appropriate error.
...
- `GET /api/tickets/:id`: Returns single ticket detail owned by `requesterId`.
- `POST /api/tickets/:id/attachments`: Uploads an attachment to ticket.
...
- **AC-03 (Cross-Requester Ticket Isolation):** Given Requester B is selected, when a request is made to access a ticket belonging to Requester A, then the system returns HTTP 403/404 and does not expose Requester A's ticket data.
...
- **AC-07 (Soft Removal with Reason):** Given an active attachment on an owned ticket, when the user requests removal and provides a valid removal reason, then `isRemoved` is set to `true`, `removedAt` is recorded, the file is displayed as "Removed", and subsequent download requests return HTTP 404/403.
...
- [ ] All 15 Acceptance Criteria (AC-01 through AC-15) are satisfied and verified by automated tests.
...
3. **Ticket Number Sequence:** Ticket numbers format sequence resets per year or increments sequentially from database records (`TKT-2026-XXXXXX`).
